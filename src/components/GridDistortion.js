...
const GridDistortion = ({
  ...
  const mouseState = useRef({ x: 0, y: 0, prevX: 0, prevY: 0, vX: 0, vY: 0, active: false });
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      ...
      mouseState.current.x = 0;
      mouseState.current.y = 0;
      mouseState.current.prevX = 0;
      mouseState.current.prevY = 0;
    };
    ...
    const animate = () => {
      ...
      // Si el ratón no está activo, solo relaja el offset para que vuelva a la normalidad
        ...
        for (let i = 0; i < size; i++) {
          for (let j = 0; j < size; j++) {
            ...
            if (distance < maxDist * maxDist) {
              ...
              data[index] += strength * 100 * mouseState.current.vX * power;
              data[index + 1] -= strength * 100 * mouseState.current.vY * power;
            }
          }
        }
      }
